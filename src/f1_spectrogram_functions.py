...
def generate_synthetic_tremor(duration, fs, f, noise_std):

    """
    Generate synthetic seismic tremor signal with Gaussian noise.

    Parameters:
        duration (float): Duration of the synthetic tremor signal in seconds.
        fs (int): Sampling frequency in Hz.
        f (float): Main frequency of the tremor signal in Hz.
        noise_std (float): Standard deviation of the Gaussian noise.

    Returns:
        numpy.ndarray: Synthetic seismic tremor signal with added Gaussian noise.

    The function generates a synthetic seismic tremor signal with the specified
    duration, sampling frequency, main frequency, and level of Gaussian noise.
    The synthetic tremor signal is represented as a 1D numpy array.
    """
    ...
    # Generate synthetic tremor signal
    tremor = np.sin(2 * np.pi * f * t)

    # Generate Gaussian noise
    noise = np.random.normal(0, noise_std, len(t))

    # Add noise to the tremor signal
    tremor_with_noise = tremor + noise

    return t, tremor_with_noise
...
def load_wf(filename, lenData, verbose=0):
...
def wf_to_H5(pathProj,dataH5_path,cat,lenData,verbose=0):
    """
...
###     clear old H5 if it exists, or else error will appear
    ...
    with h5py.File(dataH5_path,'a') as h5file:
        ...
        waveform_group = h5file.create_group("waveforms")
        ...
        for n, ev in cat.iterrows():
            if n%1000==0:
                print(n, '/', len(cat))
            data = load_wf(ev["filename"], lenData)
            if data is not None:
                                
                waveform_group.create_dataset(name=str(ev["ev_ID"]), data=data)
...
#     noverlap=args['noverlap']
    ...
    for i, evID in enumerate(evID_list):
        ...
        with h5py.File(dataH5_path,'a') as fileLoad:
            data = fileLoad["waveforms"].get(str(evID))[:]
